...
static PyObject *spectral_linspace(PyObject*self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ddLd", &start, &stop, &dim, &factor))
        return NULL;
...
static PyObject *spectral_grid(PyObject*self, PyObject *args)
{
    long long dim;
    int elliptic;
    double pixel_size, pa, elong;

    if (!PyArg_ParseTuple(args, "Ldddp", &dim, &pixel_size, &pa, &elong, &elliptic))
        return NULL;
...
static PyObject *spectral_constant_temperature(PyObject*self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Odd", &radius_obj, &stellar_radius, &stellar_temperature))
        return NULL;
    ...
    PyObject *constant_temperature_array = PyArray_SimpleNewFromData(1, &dims, NPY_DOUBLE, const_temperature_obj);

    Py_XDECREF(radius_array);

    return constant_temperature_array;
}
...
static PyObject *spectral_temperature_power_law(PyObject*self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Oddd", &radius_obj, &inner_temp, &inner_radius, &q))
        return NULL;
    ...
    return temperature_power_law_array;
}

static PyObject *spectral_surface_density_profile(PyObject*self, PyObject *args)
{
    double *radius_obj;
    double inner_radius, inner_sigma, p;

    if (!PyArg_ParseTuple(args, "Oddd", &radius_obj, &inner_radius, &inner_sigma, &p))
        return NULL;
...
static PyObject *spectral_azimuthal_modulation(PyObject*self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOdd", &xx_obj, &yy_obj, &a, &phi))
        return NULL;
    ...
    return azimuthal_modulation_array;
}
...
static PyObject *spectral_optical_thickness(PyObject*self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Od", &surface_density_profile_obj, &opacity))
        return NULL;
    ...
    return optical_thickness_array;
}
